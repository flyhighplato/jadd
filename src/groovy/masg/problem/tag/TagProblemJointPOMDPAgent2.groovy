...
class TagProblemJointPOMDPAgent2 implements TagProblemModel{
	...
	public TagProblemJointPOMDPAgent2(Integer scope = null) {
		...
		builder.addAction("act2", 4);
		builder.addActionOther("act1", 4);
		
		builder.addObservation("a2_w_pres",2);
		...
		builder.setRewardFunction(["w_row","w_col","a1_row","a1_col","a2_row","a2_col"], ["act2"], ["act1"], ) { Map variables ->
		...
		builder.addTransition(["a2_col"], ["act2"], ["act1"],  ["a2_col'"]) {Map variables ->
		...
		builder.addTransition(["a1_col"], ["act2"], ["act1"],  ["a1_col'"]) {Map variables ->
		...
		builder.addTransition(["a2_row"], ["act2"], ["act1"],  ["a2_row'"]) {Map variables ->
		...
		builder.addTransition(["a1_row"], ["act2"], ["act1"], ["a1_row'"]) {Map variables ->
		...
		builder.addTransition(["w_row","w_col"], ["act2"], ["act1"], ["w_row'","w_col'"]) {
			Map variables ->
		...
		builder.addObservation(["w_row'","w_col'","a2_row'","a2_col'"], ["act2"], ["act1"], ["a2_w_pres"]) { Map variables ->
			int w_row = variables["w_row'"]
			int w_col = variables["w_col'"]
			int a1_row = variables["a2_row'"]
			int a1_col = variables["a2_col'"]
			int w_pres = variables["a2_w_pres"]
		...
		builder.addObservation(["a2_col'"], ["act2"], ["act1"],  ["a2_col_loc"]) { Map variables ->
			int a1_col = variables["a2_col'"]
			int a1_col_loc = variables["a2_col_loc"]
		...
		builder.addObservation(["a2_row'"], ["act2"], ["act1"],  ["a2_row_loc"]) { Map variables ->
				
			int a1_row = variables["a2_row'"]
			int a1_row_loc = variables["a2_row_loc"]
